//
//  VCE.s
//  NitroGrafx Hudson/NEC HuC6260 Video Colour Encoder emulator
//
...
;@----------------------------------------------------------------------------
	ldr r1,=vcePaletteRam		;@ source RGB
	ldr r2,=MAPPED_RGB			;@ Mapping values
	mov r3,#0x200
txLoop:
	ldrh r12,[r1],#2
	mov r12,r12,lsl#1
...
;@----------------------------------------------------------------------------
							;@ https://en.wikipedia.org/wiki/YCbCr , JPEG version
	ldr r0,=vceRGBToYBYRY	;@ Destination
//	ldr r0,=vceYUVLut
	stmfd sp!,{r4-r9,lr}
	mov r8,#512
	sub r8,r8,#1
yuvLoop:					;@ map 0000000gggrrrbbb  ->  UUUUUVVVVVYYYYY
	mov r2,#0x07
	and r9,r2,r8,lsr#3		;@ R
